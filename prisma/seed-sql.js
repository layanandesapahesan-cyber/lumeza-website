...
// Use sqlite3 CLI to insert products
for (const product of products) {
  const fields = [
    'id(CUID)', 'slug', 'name', 'description', 'type', 'style', 
    'category', 'price', 'salePrice', 'images', 'fileUrl', 'tags'
  ];
  ...
  const values = [
    ...
    `'${product.type}'`,
    product.style ? `'${product.style}'` : 'NULL',
    `'${product.category}'`,
    product.price,
    product.salePrice !== null && product.salePrice !== undefined ? product.salePrice : 'NULL',
    `'${product.images.replace(/'/g, "''")}'`,
    `'${product.fileUrl.replace(/'/g, "''")}'`,
    `'${product.tags.replace(/'/g, "''")}'`
  ];
  
  try {
    execSync(`sqlite3 "prisma/dev.db" "INSERT INTO Product(slug, name, description, type, style, category, price, salePrice, images, fileUrl, tags) VALUES(${values.slice(1).join(', ')});"`, {
      stdio: 'pipe'
    });
  } catch (error) {
